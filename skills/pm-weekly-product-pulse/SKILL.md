...
trigger-phrases:
  ...
  - "weekly report"
  - "give me my Slack digest"
  - "product feedback report"
  - "what are people asking about"
  - "what's the sentiment this week"
  - "weekly roundup"
---

# PM Weekly Product Pulse

Produces a structured weekly Slack intelligence report anchored to a specific product you own.
...
```
/loop 1w /pm-weekly-product-pulse:pm-weekly-product-pulse Resell Pricing post in #private-solution-engineering
```
...
- **Slack channel to post to**: defaults to a DM to the user if not specified.
...
Confirm to the user:
> "Done — you'll get a macOS notification every [day] at [time]. When it fires, open Claude Code and run `/pm-weekly-product-pulse:pm-weekly-product-pulse`. That's it."
...
Then confirm:
> "All set! Your defaults are saved. When your reminder fires, open Claude Code and run `/pm-weekly-product-pulse:pm-weekly-product-pulse` — it'll pick up your settings automatically."
